...
import type { WorkspaceMenuState } from "./workspace-menu-types";

/** Footer Rail「Settings」单例引用保持稳定（RailMenuItem 注册用）。 */
...
function labelDot(className: string) {
  return (
    <span className={`inline-block size-2 shrink-0 rounded-full ${className}`} aria-hidden />
  );
}

export function buildWorkspaceMenuConfig(state: WorkspaceMenuState): MenuConfig {
  const { inboxRailUnread, inboxPrimaryCount, inboxReviewCount, homeDraftsChip } =
    state;

  const workspaceRailHome: RailMenuItem = {
    label: "Home",
    icon: <House />,
    sidebar: {
      items: [
        {
          ...
          menu: [
            ...
            },
            {
              ...
              chip: homeDraftsChip > 0 ? String(homeDraftsChip) : undefined,
            },
            {
  ...
  const workspaceRailInbox: RailMenuItem = {
    label: "Inbox",
    icon: railInboxIcon(inboxRailUnread),
    sidebar: {
      items: [
        {
          type: "group",
  ...
  const workspaceRailAgents: RailMenuItem = {
    label: "Agents",
    icon: <FaceRobot />,
    sidebar: {
      items: [
  ...
  const workspaceRailFlows: RailMenuItem = {
    label: "Flows",
    icon: <FolderFlows />,
    sidebar: {
      items: [
  ...
  const workspaceRailLibrary: RailMenuItem = {
    label: "Library",
    icon: <Book />,
    sidebar: {
      items: [
  ...
  const workspaceRailItems: RailMenuItem[] = [
    workspaceRailHome,
    workspaceRailInbox,
    workspaceRailAgents,
    workspaceRailFlows,
